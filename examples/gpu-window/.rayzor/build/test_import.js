...
// Run: node test_import.core.wasm  |  Open in browser with a local server
...
async function run() {
  ...
  if (isNode) {
    ...
    wasmBytes = fs.readFileSync(path.join(__dirname, "test_import.core.wasm"));
  } else {
    const resp = await fetch("test_import.core.wasm");
