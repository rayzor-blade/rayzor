...
pub fn value_to_expr(value: &MacroValue) -> Expr {
    ...
    match value {
        ...
        MacroValue::Enum(enum_name, _, _)
            if &**enum_name == "ExprDef" || &**enum_name == "Constant" =>
        {
            super::expr_adt::expr_of(value, span)
        }
        MacroValue::Object(fields) => {
...
/// An expression's `ExprDef` as a macro value; a form haxe.macro has no
/// constructor for stays an opaque `Expr`.
pub fn expr_kind_to_value(kind: &ExprKind, span: Span) -> MacroValue {
    super::expr_adt::def_of(kind, span).unwrap_or_else(|| {
