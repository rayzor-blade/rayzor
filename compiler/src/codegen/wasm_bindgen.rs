    ...
    for module in modules {
        for func in module.functions.values() {
            ...
            let class = class_map.entry(class_name.to_string()).or_insert_with(|| {
                let alloc_size = class_alloc_sizes
                    .get(class_name)
                    .copied()
                    .unwrap_or(64) as u32;
                ExportedClass {
...
fn sanitize_js_name(name: &str) -> String {
    ...
    match name {
        ...
        | "if" | "else" | "try" | "catch" | "finally" | "throw" | "typeof" | "instanceof"
        | "delete" | "void" | "in" | "of" | "with" | "yield" | "await" | "async"
        | "import" | "export" | "from" => format!("_{}", name),
        _ => name.to_string(),
    }
}
