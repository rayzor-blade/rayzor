...
/// Cache ABI ID of the compiler binary, set from `RAYZOR_BUILD_ID` env var at
/// build time (see `compiler/build.rs`). Exposed so downstream binaries (e.g.
/// `rayzor` top-level CLI) can fold it into their own cache keys without
/// needing to thread `env!` through a separate `build.rs`. Matches the value
/// used by `.blade` per-module caches in `compilation.rs`.
