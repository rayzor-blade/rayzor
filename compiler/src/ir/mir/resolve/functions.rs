...
impl<'a> HirToMirContext<'a> {
    ...
    pub(crate) fn resolve_method_function_id(
        ...
        // Keep a candidate out of the lone-name pool when its qualified name
        // clearly names a class other than the receiver's: `Linear.fromQuant`
        // must not bind to `nue.Embedding.fromQuant`. The lone-name guard
        // `func_not_other_class` cannot see qnames of ids in other modules;
        // the symbol qname is always available here.
        ...
        // The bare-name fallbacks below accept a lone same-named function. Reject a
        // match whose qname clearly belongs to another class, so an extern like
        // `NativeStackTrace.callStack` never binds to `haxe.CallStack.callStack`;
        // returning None lets the caller fall through to runtime-mapping dispatch.
        ...
        // Cross-module calls have mismatched SymbolIds but a stable qualified name,
        // so fall back to a "ClassName.methodName" lookup.
        if let Some(class_hint) = class_name_hint {
    ...
    pub(crate) fn get_or_register_extern_function(
        ...
        // A known extern's registered signature wins over the callsite's: Math
        // functions need f64 params, not the i64 inferred from the arguments.
        ...
        // An existing MIR wrapper (same name, non-empty blocks) wins over a new extern.
        ...
        // A stdlib MIR wrapper gets its body when the stdlib MIR module is
        // merged; registering the name as a Cranelift Import makes the backend
        // refuse to define that body. Detect it by the mapping flag, or by the
        // wrapper naming pattern for mapping entries that carry no flag.
        ...
        // Collect the info first: the module can't be mutated while iterating.
        ...
        if let Some((func_id, existing_param_count)) = existing_func {
            ...
            // A wider signature replaces the existing one.
        ...
        let func = crate::ir::functions::IrFunction {
            ...
            kind: crate::ir::functions::FunctionKind::ExternC,
        ...
        // The id must appear in both maps.
