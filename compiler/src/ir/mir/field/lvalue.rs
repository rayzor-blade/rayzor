...
impl<'a> HirToMirContext<'a> {
    ...
    /// lowered. Read-modify-write targets — `o.n++` — must evaluate the
    /// receiver once, so they cannot go through the `HirLValue` entry point,
    /// which lowers it itself. Sharing the one body also keeps a compound
    /// target reaching every receiver an assignment reaches: an anonymous
    /// structure, a Dynamic, a `@:cstruct`, a static.
    pub(crate) fn store_field_with_regs(
        &mut self,
        obj_reg: IrId,
        object: &HirExpr,
        field: &SymbolId,
            ...
            if is_anon {
                ...
                if let Some(field_name) = field_name {
                        ...
                        if let Some(ty_info) = type_table.get(resolved_obj_ty) {
                            ...
                            } = &ty_info.kind
                            {
                                let mut field_names: Vec<String> = anon_fields
                                    .iter()
                                    .filter_map(|f| {
                                        self.string_interner.get(f.name).map(|s| s.to_string())
                                    })
                                    .collect();
                                field_names.sort();
                    ...
                    if let Some(sorted_idx) = sorted_index {
                        ...
                        // A Float occupies the slot by its BITS -- the read
                        // side decodes with the structure's own field type and
                        // bitcasts back -- so the value register is what says
                        // so. A structure literal's field symbol carries a type
                        // that converts to neither, and taking it rounds 9.25
                        // to the integer 9, which reads back as a denormal.
                        let value_ir_type = self.builder.get_register_type(value);
                        let val_i64 = match &value_ir_type {
                            ...
                            Some(IrType::F32) => {
                                let widened = self
                                    ...
                                    .unwrap_or(value);
                                self.builder
                                    .build_bitcast(widened, IrType::I64)
                                    .unwrap_or(widened)
                            }
                            _ => self.coerce_to_i64(value, field_type_id).unwrap_or(value),
                        };
        ...
        if let Some(field_index) = field_index_opt {
                        ...
                        if let Some(val_ty) = self.builder.get_register_type(value) {
                            if val_ty != IrType::I64 {
                                match val_ty {
                                    ...
                                    IrType::Ptr(ref inner) if matches!(**inner, IrType::Void) => {
                                        ...
                                        self.builder
                                            .build_call_direct(coerce, vec![value], IrType::I64)
                                            .unwrap_or(value)
                                    }
                                    _ => self
                                        .builder
                ...
                if let Some(fname) = field_name_str {
                            ...
                            .build_call_direct(unbox_ref_id, vec![obj_reg], ptr_u8.clone())
                    {
                        ...
                        if let Some(boxed) = boxed_value {
                            let field_name_reg = self.builder.build_const(IrValue::String(fname));
                            if let Some(field_name_reg) = field_name_reg {
                                let set_field_id = self.get_or_register_extern_function(
            ...
            let field_name_str = self
                ...
                .and_then(|s| self.string_interner.get(s.name))
                .unwrap_or("<unknown>");
            let global_lookup = self.global_symbol_map.get(field).copied().or_else(|| {
                self.builder
                    .module
                    .globals
                    .values()
            ...
            self.add_error(
                &format!(
                    "Field '{}' ({:?}) index not found for write - class may not be registered",
                    field_name, field
                ),
                SourceLocation::unknown(),
            );
    ...
    /// The read counterpart of `store_index_with_regs`, so a read-modify-write
    /// target — `a[i]++`, `m[k]++` — uses one pair of registers for both halves.
    ///
    /// A Map reads through its get, which takes the key by value; every other
    /// receiver is array-style storage, read by indexing its data pointer.
    pub(crate) fn load_index_with_regs(
        &mut self,
        obj_reg: IrId,
        idx_reg: IrId,
        object_ty: TypeId,
