                            ...
                            static_stdlib_info
                        {
                            ...
                            if is_mir_wrapper {
                                let param_types: Vec<_> = arg_regs
                                    .iter()
                                    .map(|r| {
                                        self.builder.get_register_type(*r).unwrap_or(IrType::I64)
                                    })
                                    .collect();
                                let mir_func_id = self.register_stdlib_mir_forward_ref(
                                    runtime_func,
                            ...
                            if let Some(reg) = call_result {
                                if matches!(
                                    ...
                                    if let Some(hint) = self
                                        .static_factory_return_class(sc_class_name, sc_method_name)
                                    {
                            ...
                            (class_name_opt, method_name_opt)
                        {
                            ...
                            let stdlib_result = self
                                ...
                                .or_else(|| {
                                    ...
                                    let underlying_class = self.resolve_type_class_name_with(
                                        &type_table,
                                        *underlying_type,
                                    )?;
                                    self.stdlib_mapping
