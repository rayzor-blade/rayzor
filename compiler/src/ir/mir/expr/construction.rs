...
impl<'a> HirToMirContext<'a> {
    pub(crate) fn lower_new(&mut self, expr: &HirExpr) -> Option<IrId> {
        ...
        // Gates the wrap below and sizes the allocation further down. `None`
        ...
        // No constructor plus exactly one argument is a value wrap, catching
        // abstract types not detected above. It used to swallow a cross-module
        // `new C(x)` whose class lowers later; the 0- and 2-argument shapes were
        ...
        let obj_size: u64 = actual_symbol_id
            ...
            // `stage=ast` keyed by the name the wrap guard verified: a class
            // whose file lowers later has a Placeholder type and no SymbolId,
            // so that stage bails and the arg-count guess under-allocates.
            // Floored by that guess, so this can only allocate the same or more.
            .or_else(|| {
                ...
                stage.set("ast-name"); // only on success, so a miss is not mislabelled
                Some(
                    ((count as u64 + 1) * 8)
