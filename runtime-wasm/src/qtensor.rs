...
// ===========================================================================
// L4: in-guest PARALLEL Q8 flash attention (decode, seq_q == 1).
//
// Replaces the serial nue-plugins side-module kernel on the wasm in-guest path
// (this export shadows the side-module's `rayzor_tensor_flash_attn_decode_q8`
// via the wasm-linker's main-module-first resolution, once the name is dropped
// from `Q8_SIDE_FNS`). Bands the INDEPENDENT kv_heads across the in-guest
// `rayzor_thread_spawn` pool — the SAME condvar pool the matmul uses, so idle
// workers park (no oversubscription of the serial slice, unlike a spin pool).
//
// Determinism + the safety contract the matmul band path relies on: each
// kv_head is computed wholly by one worker, writes a DISJOINT output band (its
// GQA group's q_head rows), reads K/V/Q read-only from shared linear memory,
// and uses a PRE-ALLOCATED per-band scratch buffer — ZERO allocation inside
// any worker (the root cause of the reverted prior attempt). Per-block SIMD128
// math is bit-identical to the side-module kernel, so the output matches the
// serial path bit-for-bit and is run-to-run deterministic.
// ===========================================================================

const Q8_0_BS: usize = 32; // Q8_0 block size (elements)
const Q8_0_BB: usize = 34; // Q8_0 block bytes (2-byte f16 scale + 32 i8)

/// `RAYZOR_WASM_NO_FLASH_PAR=1` forces the serial in-guest Q8 flash path (for
/// A/B; the matmul thread pool is unaffected). Read once.
fn flash_par_disabled() -> bool {
    use std::sync::OnceLock;
    static OFF: OnceLock<bool> = OnceLock::new();
    *OFF.get_or_init(|| std::env::var("RAYZOR_WASM_NO_FLASH_PAR").as_deref() == Ok("1"))
}

#[cfg(target_arch = "wasm32")]
#[inline]
unsafe fn q8flash_dequant(src: *const u8, dst: &mut [f32; 32]) {
    use core::arch::wasm32::*;
    let scale = half::f16::from_bits(core::ptr::read_unaligned(src as *const u16)).to_f32();
    let sv = f32x4_splat(scale);
    let q = src.add(2) as *const i8;
    let d = dst.as_mut_ptr();
    for c in 0..2 {
        let o = c * 16;
        let q16 = v128_load(q.add(o) as *const v128);
        let lo = i16x8_extend_low_i8x16(q16);
        let hi = i16x8_extend_high_i8x16(q16);
        v128_store(
            d.add(o) as *mut v128,
            f32x4_mul(f32x4_convert_i32x4(i32x4_extend_low_i16x8(lo)), sv),
        );
        v128_store(
            d.add(o + 4) as *mut v128,
            f32x4_mul(f32x4_convert_i32x4(i32x4_extend_high_i16x8(lo)), sv),
        );
        v128_store(
            d.add(o + 8) as *mut v128,
            f32x4_mul(f32x4_convert_i32x4(i32x4_extend_low_i16x8(hi)), sv),
        );
        v128_store(
            d.add(o + 12) as *mut v128,
            f32x4_mul(f32x4_convert_i32x4(i32x4_extend_high_i16x8(hi)), sv),
        );
    }
}
...
#[cfg(target_arch = "wasm32")]
#[inline]
unsafe fn q8flash_dot(q: *const f32, k: &[f32; 32]) -> f32 {
    use core::arch::wasm32::*;
    let kp = k.as_ptr();
    ...
    for i in 0..8 {
        ...
        acc = f32x4_add(
            acc,
            f32x4_mul(
                v128_load(q.add(o) as *const v128),
                v128_load(kp.add(o) as *const v128),
            ),
        );
    }
...
#[cfg(target_arch = "wasm32")]
#[inline]
unsafe fn q8flash_axpy(out: *mut f32, w: f32, v: &[f32; 32]) {
    use core::arch::wasm32::*;
    ...
    for i in 0..8 {
        let o = i * 4;
        v128_store(
            out.add(o) as *mut v128,
            f32x4_add(
                v128_load(out.add(o) as *const v128),
                f32x4_mul(wv, v128_load(vp.add(o) as *const v128)),
            ),
        );
    }
}

/// Compute one kv_head's attention into the group's output rows, using the
/// caller-provided `scores` scratch (group*cache_len f32). Mirrors the serial
/// nue-plugins kernel block-for-block (same reduction order, same libm::expf).
#[cfg(target_arch = "wasm32")]
#[allow(clippy::too_many_arguments)]
unsafe fn q8flash_one_kv_head(
    kv_head: usize,
    group: usize,
    cache_len: usize,
    ...
    let mut kb = [0.0f32; Q8_0_BS];
    let mut vb = [0.0f32; Q8_0_BS];
    let mut gmax = [f32::NEG_INFINITY; 64];
    for i in 0..group * cache_len {
        *scores.add(i) = 0.0;
    }
    // K step.
    for l in 0..cache_len {
        let krow = k_base + l * row_bytes + kv_head * head_dim_bytes;
        for b in 0..blocks_per_head {
            q8flash_dequant((krow + b * Q8_0_BB) as *const u8, &mut kb);
            for gi in 0..group {
                let qrow = q_data.add((group_start + gi) * head_dim);
                let p = q8flash_dot(qrow.add(b * Q8_0_BS), &kb);
                *scores.add(gi * cache_len + l) += p;
            }
        }
        for gi in 0..group {
            let s = *scores.add(gi * cache_len + l) * scale;
            *scores.add(gi * cache_len + l) = s;
            if s > gmax[gi] {
                gmax[gi] = s;
            }
        }
    }
    // Softmax (libm::expf — matches the side-module fmath::exp on wasm32).
    for gi in 0..group {
        let mg = gmax[gi];
        let row = scores.add(gi * cache_len);
        let mut denom = 0.0f32;
        for l in 0..cache_len {
            let e = libm::expf(*row.add(l) - mg);
            *row.add(l) = e;
            denom += e;
        }
    ...
    for gi in 0..group {
        let orow = out_data.add((group_start + gi) * head_dim);
        for d in 0..head_dim {
            *orow.add(d) = 0.0;
        }
    }
    // V step.
    for l in 0..cache_len {
        let vrow = v_base + l * row_bytes + kv_head * head_dim_bytes;
        for b in 0..blocks_per_head {
            q8flash_dequant((vrow + b * Q8_0_BB) as *const u8, &mut vb);
            for gi in 0..group {
                let w = *scores.add(gi * cache_len + l);
                let orow = out_data.add((group_start + gi) * head_dim);
                q8flash_axpy(orow.add(b * Q8_0_BS), w, &vb);
            }
        }
    }
}

/// One band's worth of work: a contiguous kv_head range + its private scratch.
/// All pointers are usize so the struct is Send across the spawn boundary.
...
#[cfg(target_arch = "wasm32")]
unsafe fn flash_band_run(w: &FlashBandWork) {
    for kvh in w.kv_lo..w.kv_hi {
        q8flash_one_kv_head(
            kvh,
            w.group,
            w.cache_len,
...
/// Worker entry, invoked on a `rayzor_thread_spawn` OS thread via the indirect
/// table — same dispatch path as the matmul band worker.
#[cfg(target_arch = "wasm32")]
pub extern "C" fn rayzor_q8flash_band_worker(work_ptr: i32) -> i64 {
    unsafe { flash_band_run(&*(work_ptr as *const FlashBandWork)) };
    0
}

/// Parallel in-guest Q8 flash attention. Self-first ABI mirrors the side-module
/// serial kernel: `keysQ8.flashAttnDecodeQ8(q, valuesQ8, cacheLen, numQHeads,
/// scale)` -> `(k, q, v, cache_len, num_q_heads, scale)`. Returns the output
/// Tensor handle, or 0 (caller falls through to the bmm chain).
#[cfg(target_arch = "wasm32")]
#[no_mangle]
pub unsafe extern "C" fn rayzor_tensor_flash_attn_decode_q8(
    ...
    let nkvh = k.num_kv_heads;
    let hd = k.head_dim;
    if nkvh == 0 || hd == 0 || nqh % nkvh != 0 || hd % Q8_0_BS != 0 {
        return 0;
    }
    let group = nqh / nkvh;
    if group > 64 {
        return 0; // gmax stack array bound
    }
    let blocks_per_head = hd / Q8_0_BS;
    let q = &*(q_ptr as u32 as usize as *const Tensor);
    if q.dtype != DTYPE_F32 || q.data.is_null() || q.numel != nqh * hd || !q.is_contiguous() {
        return 0;
    }
    ...
    let k_base = k.data;
    let v_base = v.data;
    let q_data = q.data as usize;
    let out_data = (*out).data as usize;
    let scale_f32 = scale as f32;

    // Parallelize across kv_heads once there's enough attention work to amortize
    // the spawn/join (gate matches flash_attn_decode's cache_len>=256 crossover);
    // below that, or with <2 kv_heads, run serial on this thread.
    let env_n = guest_threads_override();
    let nthreads = if env_n > 0 { env_n.min(8) } else { 7 };
    if nkvh < 2 || cache_len < 256 || nthreads < 2 || flash_par_disabled() {
        let mut scratch = std::vec![0.0f32; group * cache_len];
        for kvh in 0..nkvh {
            q8flash_one_kv_head(
                kvh,
                group,
    ...
    // Pre-allocate N private scratch buffers (group*cache_len each) on THIS
    // thread; each band gets a disjoint slice. The Vec stays alive until join.
    let n = nthreads.min(nkvh);
    let per = group * cache_len;
    let mut scratch = std::vec![0.0f32; n * per];
    let scratch_base = scratch.as_mut_ptr();
    let band = nkvh.div_ceil(n);
    let fn_idx = rayzor_q8flash_band_worker as *const () as usize as i32;
    let mut handles: std::vec::Vec<i32> = std::vec::Vec::new();
    let mut works: std::vec::Vec<*mut FlashBandWork> = std::vec::Vec::new();
    for t in 0..n {
        let lo = (t * band).min(nkvh);
        let hi = ((t + 1) * band).min(nkvh);
        if lo >= hi {
            break;
        }
        let work = std::boxed::Box::into_raw(std::boxed::Box::new(FlashBandWork {
            ...
            blocks_per_head,
            head_dim: hd,
            head_dim_bytes: k.head_dim_bytes,
            row_bytes,
            scale: scale_f32,
            k_base,
            v_base,
            q_data,
            out_data,
            scores: scratch_base.add(t * per) as usize,
        }));
        works.push(work);
        let h = rayzor_thread_spawn(fn_idx, work as usize as i32);
        if h == 0 {
            // Spawn failed — run this band inline so the result is complete.
            flash_band_run(&*work);
        } else {
            handles.push(h);
        }
    }
    // Join before `scratch` (read by every worker) drops.
    for h in handles {
...
/// Non-wasm builds (host-side tests) never run the in-guest kernel — stub it so
/// the crate compiles for the host target.
