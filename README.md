...
1. **Correctness first.** Optimizations come after correctness is demonstrated,
   and the passes that exist for correctness run at every optimization level,
   including `-O0`. Those are drop insertion and guaranteed inlining of `inline`.
...
The diagnostic toolkit is in **[docs/CONTRIBUTING.md](docs/CONTRIBUTING.md)**:
stage dumps, MIR reading, pass bisection and the `RAYZOR_*` variables.
...
cross module boundaries; and MIR collections are ordered deliberately, so do not
